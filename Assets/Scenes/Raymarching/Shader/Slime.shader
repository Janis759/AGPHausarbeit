    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 PhongLightning(float3 position, float3 normal, float3 camPos, float3 color, float3 ro, float3 rd)
            {
                ...
                float3 ambientColor = .1 * color;

                float shadow = Shading(position, normal);
                float3 diffuseColor = .7 * color * shadow;

                float3 specularColor = float3(0, 0, 0);
                if(shadow > 0)
                {
                    float dotRV = max(0, dot(reflected, view));
                    float gloss = pow(dotRV, _SHININESS);
                    specularColor = .2 * gloss;
